...
const {listUsers, addUser, updateUser, setupCurrentIdUser} = require("./routes/user.js");
...
fs.readFile(path.join("data-base", "user.json"), { encoding: "utf-8" }, (error, data) => {
    if (!error) {
...
function processRequest(req, res) {
    ...
    if (reqUrl.pathname == "/todo") {
        ...
        switch (req.method) {
            case "GET":
                listUsers(req, res, reqUrl, todos, categories, users);
                break;
            case "POST":
                addUser(req, res, reqUrl, todos, categories, users);
                break;
            case "PUT":
                updateUser(req, res, reqUrl, users);
                break;
        }
    }
